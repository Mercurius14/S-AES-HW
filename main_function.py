...
# 实现列混淆，输入一个16位字符串，输出一个16位字符串
# def MixColumns(bits):
#     new = [XORs(bits[0], bits[6]), XORs(bits[1], bits[4], bits[7]),
#            XORs(bits[2], bits[4], bits[5]), XORs(bits[1], bits[7]),
#            XORs(bits[2], bits[4]), XORs(bits[0], bits[3], bits[5]),
#            XORs(bits[0], bits[1], bits[6]), XORs(bits[1], bits[7]),
#            XORs(bits[8], bits[14]), XORs(bits[9], bits[12], bits[15]),
#            XORs(bits[10], bits[12], bits[13]), XORs(bits[11], bits[13]),
#            XORs(bits[10], bits[12]), XORs(bits[8], bits[11], bits[13]),
#            XORs(bits[8], bits[9], bits[14]), XORs(bits[9], bits[15])]
#     return ''.join(new)
def MixColumns(bits):
...
# 实现逆列混淆，输入一个16位字符串，输出一个16位字符串
# def InvMixColumns(bits):
#     new = [XORs(bits[3], bits[5]), XORs(bits[0], bits[6]),
#            XORs(bits[1], bits[4], bits[7]), XORs(bits[2], bits[3], bits[4]),
#            XORs(bits[1], bits[7]), XORs(bits[2], bits[4]),
#            XORs(bits[0], bits[3], bits[5]), XORs(bits[0], bits[6], bits[7]),
#            XORs(bits[11], bits[13]), XORs(bits[8], bits[14]),
#            XORs(bits[9], bits[12], bits[15]), XORs(bits[10], bits[11], bits[12]),
#            XORs(bits[9], bits[15]), XORs(bits[10], bits[12]),
#            XORs(bits[8], bits[11], bits[13]), XORs(bits[8], bits[14], bits[15])]
#     return ''.join(new)
def InvMixColumns(bits):
...
# # 密文
# cipher_text = '0000011100111000'

# print(Encrypt('0110111101101011', '0000000000000000')) # 0001101110011000
# print(Encrypt('0101111101100101', '0000000000000000')) # 1101101110011000
# print(Decrypt('1101101110011000', '0000000000000000')) # 0101111101100101
...
def test():
    ...
    print("解密明文：", Decrypt(cipher_text, key))
